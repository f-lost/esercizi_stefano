...
def registrazione():
    ...
    if utente:
        ...
        if studente:
            ...
            valori = (studente[0], username, password_criptata, 0)
...
def effettua_login():
    ...
    if utente:
        ...
        if password_criptata == password_salvata[0]:
...
def inserisci_studente_utente():
    ...
    if id_studente:
       print("Studente già esistente.")
        ...
        password = username[0] + "1" + username[2] + "2" + username[4] + "3" + username[6] + "4"
        password_criptata = hashlib.md5(password.encode()).hexdigest()
        query = "insert into studenti (nome, cognome) values (%s, %s)"
        mycursor.execute(query, valore)
        mydb.commit()
...
def cambia_diritti_admin():
    username = input("Inserisci username: ")
    query = "update utenti set isadmin = not isadmin where utenti.id = select utenti.id from utenti join studenti on utenti.id = studenti.id where utenti.username = %s"
    mycursor.execute(query, (username,))
    mycursor.commit()
...
query = "create table if not exists voti (id_voti int auto_increment primary key, id int, materia varchar(50), voto float, foreign key (id) references studenti(id) on delete cascade)"
mycursor.execute(query)

query = "create table if not exists utenti (id_utenti int auto_increment primary key, id int, username varchar(50), password varchar(50), isadmin bool, foreign key (id) references studenti(id) on delete cascade)"
mycursor.execute(query)
...
elemento6_admin = Elemento("Stampa Registro Utenti", Azione(stampa_utenti))
elemento7_admin = Elemento("Rendi Admin", Azione(cambia_diritti_admin))
...
menu_utente = Menu()
elemento1_utente = Elemento("Calcola Media", Azione(media_studente))
